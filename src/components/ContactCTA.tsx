            ...
            <div>
              <div className="flex items-center justify-center">
                <button
                  ...
                  className={`inline-flex items-center gap-3 justify-center h-12 px-6 rounded-full font-semibold transition-transform transform
                    ${
                      canSubmit
                        ? "bg-gradient-to-r from-blue-500 to-blue-400 hover:scale-105 shadow-[0_10px_30px_rgba(67,56,202,0.16)] text-black"
              ...
              {status && (
                <div
                  ...
                  role="status"
                  aria-live="polite"
                  className={`text-center text-sm mt-3 ${status.startsWith("✅") ? "text-green-400" : status.startsWith("❌") ? "text-blue-400" : "text-gray-300"}`}
                >
                  {status}
                </div>
              )}
...
function FloatingInput({
  ...
  return (
    <div className="relative">
      <span className="absolute left-3 top-1/2 -tranblue-y-1/2 text-gray-400 pointer-events-none">
        {icon}
      </span>
      ...
      <input
        ...
        className={`peer w-full h-14 pl-12 pr-4 rounded-xl bg-white/5 text-white text-sm sm:text-base placeholder-transparent focus:outline-none transition-shadow shadow-inner
          ${error ? "ring-2 ring-blue-400/30" : "focus:ring-2 focus:ring-blue-400/30"}`}
      />
      ...
      {error && (
        <div id={`err-${id}`} className="mt-1 text-xs text-blue-400">
          {error}
        </div>
      )}
    </div>
