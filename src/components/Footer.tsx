...
const Footer = ({
  ...
  return (
    <motion.footer
      className="bg-gradient-to-b from-black to-teal-900 text-white"
      initial="hidden"
      whileInView="visible"
      ...
      <div className="container py-16">
        ...
        >
          ...
          <motion.div className="lg:col-span-2" variants={childVariants}>
            <h3 className="text-xl font-semibold mb-4 bg-gradient-to-r from-teal-400 to-teal-500 text-transparent bg-clip-text">
              {logoText}
            </h3>
            ...
            <div className="flex space-x-4">
              {socialLinks.map((social) => (
                <motion.a
                  key={social.name}
                  href={social.url}
                  className="inline-flex items-center justify-center w-10 h-10 bg-white/10 rounded-full text-teal-400 hover:bg-teal-400/20 transition-all"
            ...
            <div className="space-y-3">
              {quickLinks.map((link) => (
                <motion.button
                  ...
                  className="block text-gray-300 hover:text-teal-400 transition-colors text-left relative group"
                  whileHover={{ x: 5 }}
                >
                  {link.label}
                  <span className="absolute left-0 bottom-0 w-0 h-[1px] bg-teal-400 transition-all group-hover:w-full"></span>
                </motion.button>
              ))}
            </div>
          </motion.div>
        ...
        {/* Copyright */}
        <motion.div
          className="border-t border-teal-400/20 mt-12 pt-8 text-center text-gray-400"
